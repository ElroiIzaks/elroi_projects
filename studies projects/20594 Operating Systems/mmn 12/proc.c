...
int cps114()
{
	struct proc *p; /*pointer for passing proc table*/
	int pid, extpid; /*for pid & container pid*/
...
// Loop over process table looking for process with pid.
	...
	for (p = ptable.proc; p<&ptable.proc[NPROC]; p++)
	{
		...
		if (extpid == 0) extpid = pid;

		if (p->state == RUNNING)
		{
			cprintf("%s \t %d \t RUNNING \t %d \t\t %d \t %d \n",p->name,pid, extpid, (extpid != 1)? 				p->parent->pids[0].pid : 0, p->cpu_time);
